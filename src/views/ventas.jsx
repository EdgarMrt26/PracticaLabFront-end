...
const [detallesEditados, setDetallesEditados] = useState([]);
  ...
  const abrirModalActualizacion = async (venta) => {
    setCargandoDetalles(true);
    try {
      const respuestaventa = await fetch(`http://localhost:3001/api/obtenerventaporid/${venta.id_venta}`);
      ...
      const respuesta = await fetch(`http://localhost:3001/api/obtenerdetallesventa/${venta.id_venta}`);
